...
print("number of datapoints after removing none values: ", len(df))
...
x_train, x_temp, y_train, y_temp = train_test_split(
    df[['params_activations_list',
        # 'batch_size',
        # 'all_parameters', 'all_activations', 'Total_Activations_Batch_Size',
...
def encode_layer_cnn(layer_type):
    if layer_type in ['Sigmoid', 'softmax', 'ELU', 'GELU', 'Identity', 'LeakyReLU', 'Mish', 'PReLU', 'ReLU', 'SELU', 'SiLU', 'Softplus', 'Tanh']:
        return [1]
...
class LayerSequenceDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for features in sequence:
            layer_type.append(features[0])
            layer_features.append(features[1:])

        return torch.tensor(layer_features, dtype=torch.float32), torch.tensor(label, dtype=torch.long), layer_type
...
inputs, label_sample, layer_type = next(iter(train_dataloader))
print(inputs.shape, label_sample.shape, layer_type)


number_of_layer_types = max([y[0]  for x in train_dataset.x_data for y in x])
print(number_of_layer_types)
...
class MLP(nn.Module):
    def __init__(self, input_features, hidden_features, output_features, num_layers=3):
    ...
    def forward(self, x):
        out = self.model(x)
        return out  # Ensure output shape is always [batch_size, 1]


import torch.optim as optim

class ClassificationGpuUsage(pl.LightningModule):
    def __init__(self, number_of_layer_types, labels):
        super(ClassificationGpuUsage, self).__init__()
        self.number_of_layer_types = number_of_layer_types

        # Define layerwise MLPs
        self.layerwise_models = nn.ModuleDict({
            str(i): MLP(input_features=5, hidden_features=4, output_features=1, num_layers=3)
            for i in range(1, number_of_layer_types + 1)
        })

        # Define classifier
        self.classifier = nn.Linear(number_of_layer_types, len(labels))
    ...
    def forward(self, x, b):
        ...
        # Initialize layer values tensor on the correct device
        layer_val = torch.zeros(1, self.number_of_layer_types, device=x.device)

        # Accumulate layer values by layer type
        for layer_type, feature in zip(b, x):
            lt = layer_type.item()
            output = self.layerwise_models[str(lt)](feature.unsqueeze(0))
            layer_val[0, lt - 1] += output[0][0]  # Ensure the output shape is [1, 1]

        # Classify the aggregated layer values
        out = self.classifier(layer_val)
        return out
...
classifier_model = ClassificationGpuUsage(number_of_layer_types, labels)
