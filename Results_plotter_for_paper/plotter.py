...
for i, m in enumerate(methods):
    vals = df[m].values
    pos = x + (i - 1.5) * bar_w
    ax.bar(pos, np.nan_to_num(vals), bar_w, color=colors[i], edgecolor="black", linewidth=0.8, label=m)
    for xi, v in zip(pos, vals):
        if np.isnan(v):
            ax.text(xi, 2.0, "X", ha="center", va="bottom",
                    fontsize=14, fontweight="bold", color="red")
        elif v > ymax:
            ax.text(xi, ymax - 20, f"{v:.1f}GB", ha="center", va="bottom",
                    rotation=90, fontsize=9, fontweight="bold", color="black")
...
for idx, (wl, _) in enumerate(group_labels):
    if wl != prev_wl:
        ...
        prev_wl = wl
for b in boundaries:
    ax.vlines(x=b, ymin=-10, ymax=ymax, color='black', linestyle='--', linewidth=1)
...
# bottom level: batch sizes
ax.set_xticks(x)
ax.set_xticklabels([bs for _, bs in group_labels], rotation=0, fontsize=14)
...
# ax2.set_xticks(work_pos)
# ax2.set_xticklabels(work_labels, fontsize=12, fontweight="bold", rotation=30)
...
ax_workload.spines["bottom"].set_position(("outward", 45))  # adjust spacing as needed
ax_workload.set_xlim(ax.get_xlim())
ax_workload.set_xticks(work_pos)
ax_workload.set_xticklabels(work_labels, fontsize=12, fontweight="bold", rotation=30)
