...
def generate_random_mlp_config():
    input_size = int(np.random.uniform(config["input_size"]["min"], config["input_size"]["max"])) 
    ...
    # Random dropout rate
    dropout_rate = np.random.uniform(config["dropout"]["min"], config["dropout"]["max"])

    # Randomly decide whether to use dropout and batch normalization
    use_dropout = np.random.choice([True, False])
    use_batch_norm = np.random.choice([True, False])

    return input_size, output_size, depth, architecture, batch_size, activation, dropout_rate, use_dropout, use_batch_norm
...
def run_mlp_experiment(config_name, input_size, output_size, depth, architecture, batch_size, activation, dropout_rate, use_dropout, use_batch_norm):
    ...
    file_suffix = f"input:{input_size}_output:{output_size}_depth:{depth}_arch:{architecture}_batch:{batch_size}_act:{activation}_dropout:{dropout_rate}_dropout:{use_dropout}_batchnorm:{use_batch_norm}"
    ...
    train_cmd = f"CUDA_VISIBLE_DEVICES=0 python mlp_next.py --input_size {input_size} --output_size {output_size} --depth {depth} --architecture {architecture} --batch_size {batch_size} --activation {activation} --dropout_rate {dropout_rate} {'--use_dropout' if use_dropout else ''} {'--use_batch_norm' if use_batch_norm else ''}"
...
def main():
    for i in range(1, num_random_configs + 1):
        ...
        input_size, output_size, depth, architecture, batch_size, activation, dropout_rate, use_dropout, use_batch_norm = generate_random_mlp_config()
        ...
        run_mlp_experiment(config_name, input_size, output_size, depth, architecture, batch_size, activation, dropout_rate, use_dropout, use_batch_norm)
